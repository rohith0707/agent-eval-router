...
def build_agent_graph():
    return {
        "start": "plan",
        "nodes": {"plan": plan_node, "route": route_node, "tool": tool_node, "execute": execute_node, "evaluate": evaluate_node},
        "edges": {
            "plan": "route",
            "route": lambda s: "tool" if s.get("plan", {}).get("requires_tool") else "execute",
            "tool": "execute",
            "execute": "evaluate",
            "evaluate": "end",
        },
...
def _append(state: dict, step: str, status: str, detail: str) -> None:
    state.setdefault("trajectory", []).append(
        {"step": step, "status": status, "detail": detail, "iteration": state.get("iteration", 0)}
    )
...
def _failure_count(state: dict) -> int:
    return sum(
        ...
        if attempt.get("status") in {"quality_failed", "failed", "verification_incomplete"}
    )


def _should_stop(state: dict, started: float) -> tuple[bool, str]:
    if state.get("total_cost_usd", 0.0) >= state.get("max_cost_usd", 0.01):
        return True, "max_cost_exceeded"
    if state.get("iteration", 0) >= state.get("max_iterations", 3):
        return True, "max_iterations_exceeded"
    if (monotonic() - started) * 1000 >= state.get("max_wall_time_ms", 120_000):
        return True, "max_wall_time_exceeded"
    if _failure_count(state) > state.get("max_failures", 2):
        return True, "max_failures_exceeded"
    return False, ""
...
async def run_agent(
    ...
    while state.get("iteration", 0) < max_iterations:
        ...
        if stop:
            ...
            state["loop_action"] = "ABORT"
            _append(state, "loop", "aborted", reason)
            history_store.record_decision(state, action="ABORT", outcome=reason)
            break
        ...
        # Budget is a hard server-side invariant. Do not spend verification tokens
        # after the execution already consumed the configured run budget.
        if state.get("total_cost_usd", 0.0) >= max_cost_usd:
            state["status"] = "aborted"
            state["loop_action"] = "ABORT"
            _append(state, "loop", "aborted", "max_cost_exceeded")
            history_store.record_decision(state, action="ABORT", outcome="max_cost_exceeded")
            break

        await evaluate_node(state)

        if state.get("status") == "done":
            state["loop_action"] = "COMPLETE"
            _append(state, "loop", "complete", "independent verification gate passed")
            history_store.record_decision(state, action="COMPLETE", outcome="verified")
            break

        failures = _failure_count(state)
        can_repair = failures <= max_failures and state.get("iteration", 0) < max_iterations

        if can_repair:
            state["loop_action"] = "REPAIR"
            state["task"] = (
                task
                + "\n\nRepair the previous attempt. Address the verification failures "
                "and return a more complete, testable result."
            )
        ...
        state["loop_action"] = "ABORT"
        state["status"] = "aborted"
        _append(state, "loop", "stopped", "repair limits exhausted")
        history_store.record_decision(state, action="ABORT", outcome="repair_limits_exhausted")
        break

    if state.get("status") == "running":
        state["status"] = "aborted"
        state["loop_action"] = "ABORT"
        _append(state, "loop", "aborted", "loop exhausted")
    return state
