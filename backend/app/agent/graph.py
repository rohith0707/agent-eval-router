...
async def run_agent(task: str, task_type: str = "auto", max_cost_usd: float = 0.01, max_tokens: int = 512,
    ...
    state.update({"run_id": run_id, "max_cost_usd": max_cost_usd, "max_tokens": max_tokens,
                  "max_iterations": max_iterations, "max_wall_time_ms": max_wall_time_ms,
                  "max_failures": max_failures, "quality_threshold": quality_threshold,
    ...
    while state.get("iteration", 0) < max_iterations:
        ...
        if stop:
            state["status"] = "aborted"; state["loop_action"] = "ABORT"
        ...
        state["status"] = "running"; state["failure_class"] = None
        ...
        if state.get("status") == "done":
            ...
            _append(state, "loop", "complete", "verification passed")
        ...
        if failures <= max_failures and state.get("iteration", 0) < max_iterations:
            ...
            continue
        state["loop_action"] = "ESCALATE" if failures > max_failures else "ABORT"
        state["status"] = "aborted" if state["loop_action"] == "ABORT" else "escalated"
        _append(state, "loop", "stopped", state["loop_action"])
        break
