...
def evaluate_text(
    ...
    """Evaluate an output with cheap, reference-aware semantic checks.
...
def _reference_similarity(output: str, expected: str | None) -> float:
    ...
    # Numeric/reference answers are common in benchmark tasks. Require the
    # expected numeric token to survive even when the answer contains explanation.
    expected_numbers = set(re.findall(r"\d+(?:\.\d+)?", expected_normalized))
    if expected_numbers and not expected_numbers.issubset(set(re.findall(r"\d+(?:\.\d+)?", output_normalized))):
        return 0.0
...
def _structured_output_is_valid(output: str, expected: str | None) -> bool:
    # Plain-text benchmark cases are valid by default. If the expected answer is
    # JSON, validate that the response is actually parseable JSON.
