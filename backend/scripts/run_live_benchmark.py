...
async def run(limit: int) -> dict:
    ...
    for case in cases:
        ...
        state = await run_agent(task=case["task"], task_type=case.get("category", "auto"))
        results.append({
            "id": case["id"], "category": case.get("category"), "status": state.get("status"),
            "provider": state.get("provider"), "model": state.get("model"), "quality": state.get("quality"),
            "latency_ms": state.get("latency_ms"), "cost_usd": state.get("cost_usd"),
    ...
    return {
        ...
        "cases": len(results), "completed": len(completed),
        ...
        "wall_time_ms": int((perf_counter() - started) * 1000), "results": results,
    }
