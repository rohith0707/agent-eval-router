...
type BenchmarkCase = { id: string; category: string; difficulty: string; task: string; expected_behavior: string };
type BenchmarkStatus = "passed" | "failed" | "infra_failed";
type BenchmarkResult = { id: string; category: string; status: BenchmarkStatus; quality: number; latencyMs: number | null; provider: string | null; model: string | null; fallbacks: number; attempts: AttemptResult[]; output?: string; evaluation?: { mode: "reference_match" | "structural" | "semantic_overlap"; matchedSignals: number; totalSignals: number; reason: string } };

function normalize(text: string): string { return text.toLowerCase().replace(/```(?:json|sql|python)?/g, " ").replace(/[^a-z0-9$%_.<>:=/-]+/g, " ").replace(/\s+/g, " ").trim(); }
function tokenSet(text: string): Set<string> { return new Set(normalize(text).split(" ").filter(token => token.length > 1)); }
function hasAny(text: string, patterns: string[]): boolean { return patterns.some(pattern => text.includes(pattern)); }
...
export async function POST() {
  try {
    ...
    if (!smoke.result) {
      return NextResponse.json({ error: "Provider preflight failed. No configured model produced a response, so the benchmark was not run.", smoke: { attempts: smoke.attempts } }, { status: 503 });
    }
    ...
    const results = await mapWithConcurrency(cases, BENCHMARK_CONCURRENCY, async item => {
      ...
      if (!cascade.result) return { id: item.id, category: item.category, status: "infra_failed" as const, quality: 0, latencyMs: null, provider: null, model: null, fallbacks: cascade.attempts.length, attempts: cascade.attempts } satisfies BenchmarkResult;
      const evaluation = scoreByCategory(item, cascade.result.output);
      return { id: item.id, category: item.category, status: evaluation.passed ? "passed" : "failed", quality: evaluation.quality, latencyMs: cascade.result.latencyMs, provider: cascade.result.provider, model: cascade.result.model, fallbacks: cascade.attempts.filter((attempt: AttemptResult) => attempt.outcome !== "success").length, attempts: cascade.attempts, output: cascade.result.output, evaluation } satisfies BenchmarkResult;
    });

    let persisted = 0;
    if (databaseConfigured()) {
      try {
        const data = results.map(result => ({ externalId: `bench_${Date.now()}_${result.id}`, task: result.id, status: result.status === "passed" ? "passed" : "failed", selectedModel: result.model ?? "unresolved", reason: `50-case benchmark · ${result.category}`, quality: result.quality, latencyMs: result.latencyMs ?? 0, cost: 0, reliability: result.status === "passed" ? 1 : 0, candidatesJson: result.attempts, traceJson: [{ step: "Benchmark case", status: result.status, detail: result.id }, { step: "Provider cascade", status: result.status, detail: result.model ? `${result.provider} / ${result.model}` : "No candidate succeeded" }] }));
    ...
    return NextResponse.json({ suite: { name: "routing-bench-v1", cases: 50 }, durationMs: Math.round(performance.now() - started), summary: { passed: passed.length, failed: evaluatedFailures.length, infraFailed: infraFailures.length, averageQuality: Number((average(results.filter(result => result.status !== "infra_failed").map(result => result.quality)) ?? 0).toFixed(3)), passedQuality: Number((average(passed.map(result => result.quality)) ?? 0).toFixed(3)), p95LatencyMs: p95(passed.map(result => result.latencyMs ?? 0)), fallbackRate: Number((rate(results.filter(result => result.fallbacks > 0).length, results.length) ?? 0).toFixed(3)), persisted }, providerMix, byCategory: Object.fromEntries([...new Set(results.map(result => result.category))].map(category => { const categoryResults = results.filter(result => result.category === category); const evaluated = categoryResults.filter(result => result.status !== "infra_failed"); return [category, { cases: categoryResults.length, passed: categoryResults.filter(result => result.status === "passed").length, infraFailed: categoryResults.filter(result => result.status === "infra_failed").length, quality: Number((average(evaluated.map(result => result.quality)) ?? 0).toFixed(3)) }]; })), failures: [...evaluatedFailures, ...infraFailures].slice(0, 10).map(result => ({ id: result.id, category: result.category, status: result.status, attempts: result.attempts })) });
  } catch (error) {
