...
assert.match(workflow, /Deployment SHA mismatch/);
assert.match(workflow, /upload-artifact@v4/);

assert.doesNotMatch(workflow, /BASE_URL:\s*https:\/\/agent-eval-router-balsarohith5-5561s-projects\.vercel\.app/);

console.log("[production-benchmark-workflow] PASS (15 assertions) deployment-status target + fingerprint + 10 batches");
